...
const priceCellText = await page.locator("#inv-table tbody tr td").nth(5).innerText();
...
// (the tab bar is hidden while on POS now, so go via Home instead of the old nav dropdown)
await page.click("#home-btn");
await page.waitForSelector('[data-home-tile="inventory-search"]');
await page.click('[data-home-tile="inventory-search"]');
await page.waitForTimeout(300);
const stockText = await page.locator("#inv-table tbody tr td").nth(6).innerText();
...
await goTo("inventory-search", "inventory");
await page.waitForTimeout(300);
const stockAfterImport = await page.locator("#inv-table tbody tr td").nth(6).innerText();
